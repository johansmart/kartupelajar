      ...
      <div class='row'>
        <div class='col-md-12'>
          <div class='box box-info'>
            ...
            <?php
              $y=mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM menu where id='2'"));
            ?>
            <form class='form-horizontal' role='form' method=POST action='aksi/menu/menu2.php' enctype='multipart/form-data'><br>
              <input type='hidden' name='id' value='<?php echo "$y[id]";?>'>
              <div class='form-group'>
                <label class='col-sm-3 control-label'>Menu Cetak Kartu Perpustakaan : </label>
                <div class='col-sm-4'>
                  <select name="a" class="form-control">
                    <option value="<?php echo "$y[link]";?>">
                      <?php
                        if ($y[link]=='printkartuperpus')
                          { echo "Aktif"; }
                        else if ($y[link]=='blokmenucetakkaper')
                          { echo "Blokir"; };
                      ?>
                    </option>
                    <option></option>
                    <option value="printkartuperpus">Aktif</option>
                    <option value="blokmenucetakkaper">Blokir</option>
                  </select>
                </div>
              </div>
              <div class='form-group'>
